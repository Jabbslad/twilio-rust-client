...
use openapi::{
    apis as twilio,
    models::ApiPeriodV2010PeriodAccountPeriodAvailablePhoneNumberCountryPeriodAvailablePhoneNumberMobile,
};
use std::env;
...
async fn list_available_phone_number_mobile(
    config: &twilio::configuration::Configuration,
    p: &ListAvailablePhoneNumberMobileParams,
) -> Vec<String> {
    let account_id = &config.basic_auth.as_ref().unwrap().0;
    let result = match twilio::default_api::list_available_phone_number_mobile(
        config,
        account_id,
        "GB",
        p.area_code,
        ...
        p.mms_enabled,
        p.voice_enabled,
        p.exclude_all_address_required,
        p.exclude_local_address_required,
        p.exclude_foreign_address_required,
        p.beta,
        p.near_number,
        p.near_lat_long,
        p.distance,
    ...
    numbers
        .iter()
        .map(|x| x.clone().friendly_name.unwrap().unwrap())
        .collect::<Vec<String>>()
}
